...
async function assertAccountOwnership(userId, accountId) {
    ...
    const owned = await Account.findOne({
        _id: accountId,
        owner: userId,
    }).select("_id");
    if (!owned) {
...
async function ensureHeroOwnership(hero, userId) {
    ...
    if (account?.owner) {
        const ownerId =
            typeof account.owner === "object" && account.owner._id
                ? account.owner._id
                : account.owner;
...
async function findOwnedHeroById(
    id,
    userId,
    { populateAccount = false, accountFields = "username townHallLevel" } = {}
) {
    requireUserId(userId);
...
export async function getAllHeroesService(
    userId,
    { accountId, status, heroType, name }
) {
    const query = {};
    if (accountId) {
...
export async function getHeroesByAccountService(userId, accountId) {
    await assertAccountOwnership(userId, accountId);
    return Hero.find({ account: accountId }).populate(
        "account",
...
export async function startHeroUpgradeService(
    id,
...
export async function updateHeroUpgradeTimeService(id, { upgradeTime }) {
    ...
    if (hero.status !== "Upgrading") {
        const err = new Error("Hero is not upgrading");
        err.status = 400;
        throw err;
    }
    ...
    const startTime = hero.upgradeStartTime || new Date();
    hero.upgradeStartTime = startTime;
    hero.upgradeEndTime = new Date(startTime.getTime() + durationMs);
    hero.upgradeTime = Number(upgradeTime);
    await hero.save();
    return hero;
}
...
export async function completeHeroUpgradeService(id, userId) {
    ...
    await Account.findByIdAndUpdate(hero.account, {
        $inc: { totalUpgrades: 1 },
    });
    return hero;
}
...
export async function getHeroUpgradeProgressService(id, userId) {
    ...
    const elapsed =
        now.getTime() - (hero.upgradeStartTime?.getTime() || now.getTime());
    const progress =
        totalMs > 0 ? Math.min(100, Math.max(0, (elapsed / totalMs) * 100)) : 0;
    const remaining = Math.max(
        0,
        (hero.upgradeEndTime?.getTime() || now.getTime()) - now.getTime()
    );
...
export async function validateHeroUpgradeService(id, userId) {
    const hero = await findOwnedHeroById(id, userId);
    const canUpgrade =
        hero.status === "Idle" && hero.currentLevel < hero.maxLevel;
    return {
        canUpgrade,
...
export async function getTopHeroesService(accountId, userId) {
    await assertAccountOwnership(userId, accountId);
    return Hero.find({ account: accountId }).sort({
        currentLevel: -1,
        name: 1,
    });
}
