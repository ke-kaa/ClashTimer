...
async function findOwnedPetById(userId, petId) {
    ...
    const accountOwned = await Account.exists({
        _id: pet.account,
        owner: userId,
    });
    if (!accountOwned) {
...
export async function unlockPetService(
    userId,
    accountId,
    { petType, petName, name } = {}
) {
    ...
    const matchedConfigEntry = petsAvailable.find(
        (entry) => normalizePetInput(entry.name) === normInput
    );
    if (!matchedConfigEntry) {
        const available = petsAvailable.map((p) => p.name).filter(Boolean);
        const err = createError(
            "Pet not available at this Town Hall level",
            404
        );
...
export async function assignPetToHeroService(
    userId,
    accountId,
    { petId, heroId } = {}
) {
...
export async function startPetUpgradeService(
    userId,
    accountId,
    { petId, upgradeTime, upgradeCost } = {}
) {
    await assertAccountOwnership(userId, accountId);
    if (!petId) {
        throw createError("petId required", 400);
    ...
    if (upgradeTime === undefined || upgradeTime === null) {
        throw createError("upgradeTime required (seconds)", 400);
    }

    const timeNum = Number(upgradeTime);
...
export async function updatePetUpgradeTimeService(
    userId,
    accountId,
    { petId, upgradeTime }
) {
    ...
    if (pet.status !== "Upgrading") {
        throw createError("Pet is not upgrading", 409);
    }
    if (pet.currentLevel >= pet.maxLevel) {
        throw createError("Pet already at max level", 400);
    }

    const startTime = pet.upgradeStartTime || new Date();
    pet.upgradeStartTime = startTime;
    pet.upgradeEndTime = new Date(startTime.getTime() + durationMs);
    pet.upgradeTime = Number(upgradeTime);
    await pet.save();
    return pet;
}
...
export async function getPetUpgradeStatusService(userId, petId) {
    ...
    return {
        petId: pet._id,
        status:
            now >= pet.upgradeEndTime.getTime()
...
export async function finishPetUpgradeService(
    userId,
    accountId,
    { petId } = {}
) {
...
export async function cancelPetUpgradeService(
    userId,
    accountId,
    { petId } = {}
) {
