...
async function findOwnedTroopById(userId, troopId) {
    ...
    const ownsAccount = await Account.exists({ _id: troop.account, owner: userId });
    if (!ownsAccount) {
...
export async function createTroopService(userId, accountId, { name, troopName, currentLevel } = {}) {
    const account = await assertAccountOwnership(userId, accountId, 'townHallLevel troops');
    ...
    if (!thConfig || !Array.isArray(thConfig.troops)) {
        throw createError(`No troop configuration for Town Hall ${thLevel}`, 400);
    }

    const targetKey = normalizeKey(inputName);
    const troopEntry = thConfig.troops.find(s => normalizeKey(s.troopName ?? s.name) === targetKey);
    if (!troopEntry) {
        const err = createError('troop not available at this Town Hall', 404);
        err.availableTroops = thConfig.troops.map(s => s.troopName ?? s.name).filter(Boolean);
        throw err;
    }
    ...
    const duplicate = await Troop.findOne({ account: accountId, name: canonicalName });
    if (duplicate) {
    ...
    if (Array.isArray(account.troops)) {
        ...
        await Account.findByIdAndUpdate(accountId, { $addToSet: { troops: troop._id } });
    }
...
export async function deleteTroopService(userId, troopId) {
    ...
    await Account.findByIdAndUpdate(troop.account, { $pull: { troops: troop._id } });
    await troop.deleteOne();
    return { deleted: true, troopId: troop._id };
}

export async function startTroopUpgradeService(userId, accountId, { troopId, upgradeTimeSec, upgradeCost } = {}) {
    await assertAccountOwnership(userId, accountId);
    if (!troopId) {
...
export async function finishTroopUpgradeService(userId, accountId, { troopId } = {}) {
    await assertAccountOwnership(userId, accountId);
    if (!troopId) {
        throw createError('troopId required', 400);
    }
...
export async function cancelTroopUpgradeService(userId, accountId, { troopId } = {}) {
    await assertAccountOwnership(userId, accountId);
    if (!troopId) {
        throw createError('troopId required', 400);
    }
