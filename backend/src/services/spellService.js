...
export async function createSpellService(
    userId,
    accountId,
    { name, spellName, currentLevel } = {}
) {
    const account = await assertAccountOwnership(
        userId,
        accountId,
        "townHallLevel spells"
    );
    ...
    if (!thConfig || !Array.isArray(thConfig.spells)) {
        throw createError(
            `No spell configuration for Town Hall ${thLevel}`,
            400
        );
    }

    const targetKey = normalizeKey(inputName);
    const spellEntry = thConfig.spells.find(
        (s) => normalizeKey(s.spellName ?? s.name) === targetKey
    );
    if (!spellEntry) {
        const err = createError("Spell not available at this Town Hall", 404);
        err.availableSpells = thConfig.spells
            .map((s) => s.spellName ?? s.name)
            .filter(Boolean);
        throw err;
    }
    ...
    const duplicate = await Spell.findOne({
        account: accountId,
        name: canonicalName,
    });
    if (duplicate) {
...
export async function deleteSpellService(userId, spellId) {
    ...
    await Account.findByIdAndUpdate(spell.account, {
...
export async function updateSpellUpgradeTimeService(
    userId,
    accountId,
    { spellId, upgradeTimeSec } = {}
) {
    await assertAccountOwnership(userId, accountId);
    if (!spellId) {
    ...
    if (spell.status !== "Upgrading") {
        throw createError("Spell is not upgrading", 409);
    }
    ...
    const startTime = spell.upgradeStartTime || new Date();
    spell.upgradeStartTime = startTime;
    spell.upgradeEndTime = new Date(startTime.getTime() + durationMs);
    spell.upgradeTime = Number(upgradeTime);
    await spell.save();
    return spell;
}

export async function startSpellUpgradeService(
    userId,
    accountId,
    { spellId, upgradeTimeSec, upgradeCost } = {}
) {
    await assertAccountOwnership(userId, accountId);
    if (!spellId) {
        throw createError("spellId required", 400);
...
export async function finishSpellUpgradeService(
    userId,
    accountId,
    { spellId } = {}
) {
...
export async function cancelSpellUpgradeService(
    userId,
    accountId,
    { spellId } = {}
) {
