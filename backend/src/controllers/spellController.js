...
function handleError(res, error, fallbackMessage = "Request failed") {
    ...
    const payload = {
        ...
        ...(error?.availableSpells
            ? { availableSpells: error.availableSpells }
            : {}),
    };
...
export async function createSpellController(req, res) {
    try {
        ...
        const spell = await createSpellService(
            getUserId(req),
            accountId,
            req.body
        );
...
export async function getSpellsController(req, res) {
    try {
        const accountId = getAccountId(req);
        const spells = await getSpellsByAccountService(
            getUserId(req),
            accountId
        );
...
export async function startSpellUpgradeController(req, res) {
    try {
        ...
        const result = await startSpellUpgradeService(
            getUserId(req),
            accountId,
            {
                ...req.body,
...
export async function updateSpellUpgradeTimeController(req, res) {
    try {
        const accountId = getAccountId(req);
        const spellId = req.params.spellId || req.body?.spellId;
        const result = await updateSpellUpgradeTimeService(
            getUserId(req),
            accountId,
            {
                ...req.body,
        ...
        return handleError(res, error, "Failed to update upgrade time.");
    }
}
...
export async function finishSpellUpgradeController(req, res) {
    try {
        const accountId = getAccountId(req);
        const spellId = req.params.spellId || req.body?.spellId;
        const spell = await finishSpellUpgradeService(
            getUserId(req),
            accountId,
            {
                ...req.body,
...
export async function cancelSpellUpgradeController(req, res) {
    try {
        const accountId = getAccountId(req);
        const spellId = req.params.spellId || req.body?.spellId;
        const spell = await cancelSpellUpgradeService(
            getUserId(req),
            accountId,
            {
                ...req.body,
...
export async function getSpellUpgradeStatus(req, res) {
    try {
        ...
        const status = await getSpellUpgradeStatusService(
            getUserId(req),
            spellId
        );
