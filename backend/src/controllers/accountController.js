import {
    updateAccountDataFromPasteService,
    addVillageDetailService,
    getVillageService,
    getAccountsForDashboardService,
    getAccountsService,
...
export async function updateAccountDataFRomPasteController(req, res) {
    try {
        ...
        if (!pastedData)
            return res
                .status(400)
                .json({ error: "Paste data is required in request body" });
        const updatedAccount = await updateAccountDataFromPasteService(
            pastedData
        );
...
export async function getAccounts(req, res) {
    try {
        ...
        let accounts = await getAccountsService({
            userId,
            clanTag,
            townHallLevel,
            playerTag,
...
export async function createAccount(req, res) {
    try {
        ...
        if (!userId)
            return res.status(401).json({ error: "Authentication required" });
        const { username, playerTag, townHallLevel, clanTag, preferences } =
            req.body;
        ...
        if (
            ...
            return res
                .status(400)
                .json({ error: "Invalid username or town hall level." });
        }

        let account = await createAccountService({
            userId,
            username,
            playerTag,
            townHallLevel,
...
export async function updateAccount(req, res) {
    try {
        ...
        let account = await updateAccountService({
            userId,
            id,
            username,
            playerTag,
        ...
        if (error.message === "Account not found")
            return res.status(404).json({ error: error.message });
        if (error.message === "Player tag already exists")
            return res.status(400).json({ error: error.message });
        if (error.message === "Invalid town hall level")
            return res.status(400).json({ error: error.message });
        if (error.message === "Changing Town Hall level is not supported yet.")
            return res.status(400).json({ error: error.message });
        console.log(error);
        return res.status(500).json({ error: error.message });
    }
...
export async function getAccountsByClan(req, res) {
    try {
        ...
        const accounts = await getAccountsByClanService({
            userId,
            clanTag,
            sortBy,
        });
...
export async function updateAccountPreferences(req, res) {
    try {
        ...
        const userId = req.user?.id || req.user?._id;
        const updated = await updateAccountPreferencesService({
            userId,
            accountId: id,
            preferences,
        });
...
export async function searchAccounts(req, res) {
    try {
        ...
        const userId = req.user?.id || req.user?._id;
        const accounts = await searchAccountsService({
            userId,
            q,
            type,
