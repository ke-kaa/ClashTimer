...
import {
    unlockSiegeService,
    startSiegeUpgradeService,
    finishSiegeUpgradeService,
    getSiegeUpgradeStatusService,
    cancelSiegeUpgradeService,
    updateBuildingUpgradeTimeService,
} from "../services/siegeService.js";
...
export async function getSiegesByAccountId(req, res) {
    try {
        ...
        const sieges = await Siege.find({ account: accountId }).sort({
            createdAt: 1,
        });
...
export async function unlockSiege(req, res) {
    try {
        ...
        if (!accountId || !input) {
            return res
                .status(400)
                .json({ error: "accountId and siegeType (or name) required" });
        }
        ...
        if (e.message === "Siege not available at this Town Hall") {
            return res
                .status(404)
                .json({ error: e.message, availableSieges: e.availableSieges });
        }
        console.log(e.message);
        return res.status(500).json({ error: "Internal server error." });
    }
...
export async function startSiegeUpgrade(req, res) {
    try {
        ...
        const { upgradeTime, upgradeCost = 0 } = req.body;

        if (!siegeId) {
            return res.status(400).json({ error: "siegeId required" });
        }
        if (upgradeTime == null || isNaN(upgradeTime) || upgradeTime < 0) {
            return res.status(400).json({ error: "upgradeTime must be >= 0" });
        }

        const result = await startSiegeUpgradeService(
            siegeId,
            upgradeTime,
            upgradeCost
        );
        ...
        if (
            e.message.includes("already upgrading") ||
            e.message.includes("max level")
        ) {
            return res.status(409).json({ error: e.message });
        }
        console.log(e.message);
...
export async function updateSiegeUpgradeTimeController(req, res) {
    try {
        const siegeId = req.params.id || req.body?.siegeId;
        const { upgradeTime, upgradeCost = 0 } = req.body;
        ...
        const result = await updateBuildingUpgradeTimeService(
            siegeId,
            upgradeTime,
            upgradeCost
        );
